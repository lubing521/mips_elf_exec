...
ngx_http_mp4_handler(ngx_http_request_t *r)
{
    ...
    if (r->args.len && (ngx_strstr(r->uri.data, "ipad_file=_") == NULL)) {
        ...
        if (ngx_http_arg(r, (u_char *) "start", 5, &value) == NGX_OK ||
            ...
            memcpy(temp, value.data, value.len);
...
//            start = (int) (strtod((char *) value.data, NULL) * 1000);
            /*
             * zhaoyao XXX: 在设备上编译并执行的strtod工作异常，转而自己实现函数str_to_mp4_para，
             *              处理和计算mp4的参数。
             */
            start = str_to_mp4_para(temp);
            /*
             * zhaoyao XXX FIXME: 当start为0时，mp4模块返回从0x100000开始的数据，目前没有查明原因，
             *                    采用规避方法，即当start为0时，强制加上0.1秒使其不为0.
             */
            if (start == 0) {
                start += 100;
            }
