...
void ngx_dbg_conn_peer_info(ngx_peer_connection_t *peer, char *s, int d)
{
    ...
    return;
    printk_rt("dbg connectine peer info:%s - %d\n", s, d);
    if (peer->name) {
        printk_rt("srv name:%s\n",peer->name->data);
    } else {
        printk_rt("srv name is NULL\n");
    }

    if (peer->sockaddr) {
        add_in = (struct sockaddr_in *) peer->sockaddr;
        printk_rt("srv sock add: %a, %u\n", 
            add_in->sin_addr, add_in->sin_port);
    } else {
        printk_rt("srv sock add is NULL\n");
    }

    if (peer->local) {
        if (peer->local->sockaddr) {
            add_in = (struct sockaddr_in *) peer->local->sockaddr;
            printk_rt("local sockaddr: %a, %u\n", 
                add_in->sin_addr, add_in->sin_port);
        } else {
            printk_rt("peer->local->sockaddr is NULL\n");
        }
    } else {
        printk_rt("peer->local is NULL\n");
    }

    TRACE_SF();
}

void ngx_dbg_sockaddr(struct sockaddr *sockaddr, char *s, int d)
{
    struct sockaddr_in *add_in;
    printk_rt("dbg sockaddr info:%s - %d\n", s, d);
    if (sockaddr) {
        add_in = (struct sockaddr_in *) sockaddr;
        
        printk_rt("sa_family:%d, sockaddr: %a, %u\n",sockaddr->sa_family,
            add_in->sin_addr, add_in->sin_port); 
    } else {
        printk_rt("sockaddr is NULL\n");
    }
}
void ngx_dbg_conn_info(ngx_connection_t *c, char *s, int d)
{
    struct sockaddr_in *add_in;
return;
    printk_rt("dbg connectine info:%s - %d\n", s, d);
    printk_rt("addr_text:%s\n",c->addr_text.data);
    add_in = (struct sockaddr_in *) c->local_sockaddr;
    printk_rt("local_sock: %a, %u\n", 
        add_in->sin_addr, add_in->sin_port);

    add_in = (struct sockaddr_in *) c->sockaddr;
    printk_rt("sockaddr: %a, %u\n", 
        add_in->sin_addr, add_in->sin_port); 
...
ngx_http_upstream_test_connect(ngx_connection_t *c)
{
...
#endif
    {
        ...
        FD_SET (c->fd, &w_set);
        /*
         * 非阻塞连接，先判断该连接上是否有可读事件(连接建立成功，或者异常),
         * 再通过SO_ERROR选项决断连接是否异常 
         */
        for(;;) {
            ret = select (c->fd + 1, NULL, &w_set, NULL, &tv);
            if (ret != -1 || ngx_errno != EINTR) {
                break;
            }
        }
    ...
    ngx_uint_t ft_type)
{
    ...
    if (u->peer.sockaddr) {
        ...
        u->peer.free_peer(&u->peer, u->peer.data, state);
    ...
    ngx_http_upstream_t *u, ngx_int_t rc)
{
    ...
    if (u->peer.free_peer && u->peer.sockaddr) {
        u->peer.free_peer(&u->peer, u->peer.data, 0);
        u->peer.sockaddr = NULL;
    }
