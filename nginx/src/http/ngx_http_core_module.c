    ...
    size_t *root_length, size_t reserved)
{
    ...
    if (r->getfile) {
        /* zhaoyao: URI is like this /getfile?222.73.245.205/youku/2180E8ADC6A6.flv?urlID=xxx */
        if ((value.data = ngx_strstr(r->args.data, "urlID")) != NULL) {
            value.data = value.data + 6;
            for (value.len = 0; isdigit(value.data[value.len]); value.len++) {
                ;
            }
            url_id = ngx_atoi(value.data, value.len);
        } else {
            ngx_log_stderr(NGX_OK, "%s ERROR: \"urlID\" miss!!!\n", __func__);
            goto default_way;
        }

        if (url_id < 0) {
            ngx_log_stderr(NGX_OK, "%s ERROR: invalid url id %d\n", __func__, url_id);
            goto default_way;
        }

        local_path.data = str;
        local_path.len = sizeof(str);
        if (ngx_http_sp_id2fpath(url_id, &local_path) != 0) {
            ngx_log_stderr(NGX_OK, "%s ERROR: get local path failed\n", __func__);
            goto default_way;
        }

        *last++ = '/';
        last = ngx_cpystrn(last, local_path.data, local_path.len + 1);
    } else {
default_way:
    ...
    u_char *xff, size_t xfflen, ngx_array_t *proxies, int recursive)
{
    ...
    if (family == AF_INET) {
        inaddr = ((struct sockaddr_in *) addr->sockaddr)->sin_addr;
    }
