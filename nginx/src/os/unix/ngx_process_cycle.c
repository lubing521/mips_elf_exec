...
static void ngx_master_process_cycle_timer(ulong arg)
{
    task_t task_master = (task_t)arg;
    resum(task_master);
    return ;
}
...
*/
    ...
    for ( ;; ) {
        if (delay) {
            ...
            /*tangyoucan add start*/
            task_delay = itv.it_value.tv_sec * HZ + (itv.it_value.tv_usec *HZ)/1000;
	        init_timer(&age_timer);
	        age_timer.expires = task_delay;
	        age_timer.data = task_master;
	        age_timer.function = ngx_master_process_cycle_timer;
	        add_prio_timer(&age_timer, APP_TPRIO);
            /*tangyoucan add end*/
            //tangyoucan delete start
            /*if (setitimer(ITIMER_REAL, &itv, NULL) == -1) {
                ngx_log_error(NGX_LOG_ALERT, cycle->log, ngx_errno,
                              "setitimer() failed");
            }*/
            //tangyoucan delete end

        }

        ngx_log_debug0(NGX_LOG_DEBUG_EVENT, cycle->log, 0, "sigsuspend");

        /*sigsuspend(&set); tangyoucan*/
        suspend();
        ngx_time_update();
