...
ngx_resolver_send_query(ngx_resolver_t *r, ngx_resolver_node_t *rn)
{
    ...
    if (rn->naddrs == (u_short) -1) {
        /* XXX:zhouzibo, UDB非阻塞有问题，暂时用阻塞方便代替 */
        //n = ngx_send(uc->connection, rn->query, rn->qlen);
        n = sendto(uc->connection->fd, rn->query, rn->qlen, 0, 
...
ngx_resolver_read_response(ngx_event_t *rev)
{
    ...
    extern ngx_uint_t ngx_http_resolver;
    memset(&dest_addr,0,sizeof(struct sockaddr_in));
    dest_addr.sin_family = AF_INET;
    dest_addr.sin_port = htons(53);
    dest_addr.sin_addr = ngx_http_resolver;
/*
    if(inet_aton("192.168.58.110", (struct in_addr *)&dest_addr.sin_addr)<0){
        printk_rt("error, %s ,%d\n", __FILE__, __LINE__);
    }
*/
    do {
        //n = ngx_udp_recv(c, buf, NGX_RESOLVER_UDP_SIZE);
        {
            ...
            FD_SET (c->fd, &r_set);
            ret = select (c->fd + 1, &r_set, NULL, NULL, &tv);
            if (ret <= 0) {
                return;
            }
            n = recvfrom(c->fd, buf,NGX_RESOLVER_UDP_SIZE,  0,  &dest_addr, sizeof(dest_addr));
        }
        if (n < 0) {
            return;
        }
