...
/*
 * zhaoyao XXX: str的格式为a.b，例如5.18，将a.b乘以1000再转换为int
 */
ngx_int_t str_to_mp4_para(char *str)
{
    ngx_int_t a = 0, b = 0, ret = 0, i = 3;
    char temp_str[32], *p, *q;
    ...
    for (p = str, q = temp_str; *p != '\0' && *p != '.'; p++, q++) {
		if (!isdigit(*p)) {
			return 0;   /* zhaoyao: default value */
		}
        *q = *p;
    }
    a = atoi(temp_str);
    ...
    memset(temp_str, 0, sizeof(temp_str));
    for (p = p + 1, q = temp_str; *p != '\0' && i > 0; p++, q++, i--) {
		if (!isdigit(*p)) {
			return 0;   /* zhaoyao: default value */
		}
        *q = *p;
